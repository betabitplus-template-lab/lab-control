...
|---|---:|---|---|---|---|---|
| `experiments/minimal-policy/py_lib_policy.py` | 191 | Python stdlib | organization-specific `_api`/`_internal`, console-script, root-init, dynamic import, example and skeleton conventions | generic import tools do not express TOML entry-point targets, declaration-only AST shape, `# %%` first-line or required docs paths together | low; one AST/fs pass, six focused tests | one CLI and pre-commit hook |
...
| `ensure-promotion-pr.yml` | 26 | GitHub Actions/CLI | create at most one `dev → main` PR when a diff exists | GitHub can merge/queue an existing PR but does not create this promotion PR automatically | low; stateless/idempotent | reusable workflow |
...
- Mandatory lifecycle coordination without workflow updater: **52 LOC**.
- Preferred lifecycle surface including the separate updater: **151 LOC**.
- Generic wiring guard: **12 LOC**.
- Unique policy: **191 LOC**.
- Total direct executable custom non-product code: **354 LOC**.
