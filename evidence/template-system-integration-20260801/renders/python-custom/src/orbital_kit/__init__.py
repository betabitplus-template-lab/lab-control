"""Supported public package entrypoint for `orbital_kit`.

Why:
    Exposes the stable public surface from one import boundary.

What belongs here:
    Re-exports of facade functions/classes, public DTOs, config objects,
    vocabulary types, public exceptions, and package version.

What does not belong here:
    Raw defaults, private runtime helpers, adapters, stores, or other
    implementation details.
"""

from __future__ import annotations

from importlib.metadata import PackageNotFoundError, version

from orbital_kit._api.config import (
    OrbitalKitConfig,
    get_config,
    install_config,
)
from orbital_kit._api.errors import (
    InvalidConfigValueError,
    OrbitalKitError,
)

try:
    __version__ = version("orbital-kit")
except PackageNotFoundError:  # pragma: no cover
    __version__ = "0.0.0+local"

__all__ = [
    "InvalidConfigValueError",
    "OrbitalKitConfig",
    "OrbitalKitError",
    "__version__",
    "get_config",
    "install_config",
]
