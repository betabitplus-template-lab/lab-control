...
Use [docs/orbital_kit/README.md](docs/orbital_kit/README.md) for package
docs, [tests/README.md](tests/README.md) for test-tree layout, and
[docs/orbital_kit/verification/README.md](docs/orbital_kit/verification/README.md)
...
```bash
uvx --from copier==9.16.0 copier check-update
```

Apply the latest released Ternforge template:

```bash
uvx --from copier==9.16.0 copier update
```
...
```bash
uv run pytest tests/orbital_kit
```

Run only hermetic tests:

```bash
uv run pytest tests/orbital_kit -m hermetic
```
...
```bash
direnv exec . uv run python examples/orbital_kit/<module>.py
```

Keep examples focused on imports from `orbital_kit`. If an example
...
```bash
direnv exec . uv run python -m workbench.orbital_kit.<module>
```
