...
description: High-level system story for orbital kit. Use when you need the integrated architecture narrative before reading focused docs.
---
...
Orbital Kit starts with the common py library foundation: callers
import supported names from `orbital_kit`; `_api/config.py` re-exports
...
```mermaid
flowchart LR
    Caller["Caller"] --> Public["orbital_kit"]
...
The baseline package exports `__version__`, `OrbitalKitError`,
`InvalidConfigValueError`, `OrbitalKitConfig`, `get_config`, and
