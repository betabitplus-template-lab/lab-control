...
```bash
direnv exec . uv run pytest tests/orbital_kit -m hermetic -q
direnv exec . uv run pytest tests/orbital_kit/e2e/public_boundary -q --no-cov
```
