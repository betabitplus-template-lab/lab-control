...
description: High-level system story for sample lib. Use when you need the integrated architecture narrative before reading focused docs.
---
...
Sample Lib starts with the common py library foundation: callers
import supported names from `sample_lib`; `_api/config.py` re-exports
...
```mermaid
flowchart LR
    Caller["Caller"] --> Public["sample_lib"]
